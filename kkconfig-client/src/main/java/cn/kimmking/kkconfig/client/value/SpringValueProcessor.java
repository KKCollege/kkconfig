...
import cn.kimmking.kkconfig.client.util.PlaceholderHelper;
...
public class SpringValueProcessor implements BeanPostProcessor, BeanFactoryAware, ApplicationListener<EnvironmentChangeEvent> {
    ...
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        FieldUtils.findAnnotatedField(bean.getClass(), Value.class).forEach(
          field -> {
              log.info("[KKCONFIG] >> find spring value: {}", field);
              Value value = field.getAnnotation(Value.class);
              helper.extractPlaceholderKeys(value.value()).forEach(key -> {
                          log.info("[KKCONFIG] >> find spring value: {}", key);
    ...
    public void onApplicationEvent(EnvironmentChangeEvent event) {
        log.info("[KKCONFIG] >> update spring value for keys: {}", event.getKeys());
        event.getKeys().forEach(key -> {
            log.info("[KKCONFIG] >> update spring value: {}", key);
            ...
            springValues.forEach(springValue -> {
                log.info("[KKCONFIG] >> update spring value: {} for key {}", springValue, key);
                        try {
                            ...
                            log.info("[KKCONFIG] >> update value: {} for holder {}", value, springValue.getPlaceholder());
                            springValue.getField().setAccessible(true);
                            springValue.getField().set(springValue.getBean(), value);
                        } catch (Exception ex) {
                            log.error("[KKCONFIG] >> update spring value error", ex);
                        }
                    }
            );
        });
